...
def lambda_handler(event, context):
    ...
    token_url = "https://btkanobeta.service-now.com/oauth_token.do"
    test_api_url = "https://btkanobeta.service-now.com/api/x_bttm_bt_tmf642_e/tmf642_eventmgmt/stacks"
    ...
    with open('alarm.json','r') as input_file:
        ...
        if data['event']['alarm']['specificProblem'] in ["Problem"]:
        ...
        if data['event']['alarm']['perceivedSeverity'] in [""]:
            data['event']['alarm']['perceivedSeverity'] = str(severity)
    ...
    access_token_response = requests.post("https://btkanobeta.service-now.com/oauth_token.do", 
   ...
   # Sending Email Notification through SNS
    ...
    MY_SNS_TOPIC_ARN = 'arn:aws:sns:eu-west-2:294773374473:App15549_Simplify-Core-Services_ECS-Beta_DevOps_Mail_Topic'
    ...
    elif(metric_namespace == "CCTA-Coreservices-Integration"):
        sns_client.publish(
        TopicArn = MY_SNS_TOPIC_ARN,
        Subject = str(subject),
